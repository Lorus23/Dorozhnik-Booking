...
@section('content')
    ...
    @can('checkin_delete')
        ...
        <ul class="list-inline">
            <li><a href="{{ route('admin.checkins.index') }}"
                   style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">Check-In</a></li>
            |
            <li><a href="{{ route('admin.checkins.index') }}?show_deleted=1"
                   style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">Check-Out</a></li>
        </ul>
        </p>
    @endcan
    ...
    <div class="panel panel-default">
        ...
        'route' => ['admin.checkins.perma_del', $checkin->id])) !!}
                            ...
                            @else
                                <td>
                                    @can('checkin_view')
                                        <a href="{{ route('admin.checkins.show',[$checkin->id]) }}"
                                           class="btn btn-xs btn-primary">@lang('quickadmin.qa_detail')</a>
                                    @endcan
                                    @can('checkin_edit')
                                        <a href="{{ route('admin.checkins.edit',['checkin_id'=>$checkin->id]) }}"
